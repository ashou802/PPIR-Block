...
class UpsampleWithGuidance(nn.Module):
    ...
    def forward(self, x, guidances):
        out = x
        # For each stage of upsampling, combine with respective guidance feature
        for i in range(self.num_stages):
            guidance = guidances[i]
            combined = torch.cat((out, guidance), dim=1)  # concatenate along the channel dimension
            reduced = self.reduce_blocks2[i](combined)    # reduce channels
...
class CotGuidanceBlock(nn.Module):
    ...
    def forward(self, x):
        guidances = []  # 临时存储本次前向传播的指导特征
        # 在每次转置卷积中进行输入
        for conv in self.ConvTransposes:
            # 进行转置卷积
            x = conv(x)
            # 保存每次转置卷积的结果并进行下采样
            downsampled = self.down_sample3(x)
            guidances.append(downsampled)

        return guidances  # 返回本次前向传播的指导特征
...
class SwinIR(nn.Module):
    def __init__(self, img_size=64, patch_size=1, in_chans=3,
        ...
        self.window_size = window_size
        self.CotGuidanceBlock = CotGuidanceBlock(num_feat, upscale)
        self.UpsampleWithGuidance = UpsampleWithGuidance(upscale, num_feat)

        #####################################################################################################
        ################################### 1, shallow feature extraction ###################################

        self.conv_3p3 = nn.Conv2d(num_in_ch, num_feat, 3, 1, 1)

        #self.depth_conv = nn.Conv2d(embed_dim, embed_dim, kernel_size=3, padding=1, bias=True, groups=num_in_ch)
        self.down_sample = nn.Conv2d(num_feat, num_feat, kernel_size=3, stride=2, padding=1)
    ...
    def forward(self, x):
        ...
        # 获取指引特征
        guidance = self.CotGuidanceBlock(C2)
        guidance_list = []

        # 根据不同放大倍数获取不同的指导特征
        if self.upscale in [2, 3]:
            # 这里注意逻辑与运算符的优先级，要用括号
            guidance_list.append(guidance[0])
        if self.upscale >= 4:
            guidance_list.append(guidance[0])
            guidance_list.append(guidance[1])
        if self.upscale == 8:
            guidance_list.append(guidance[2])
        # 调用 UpsampleWithGuidance 并传入指导特征列表
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            x = self.conv_last(self.UpsampleWithGuidance(x, guidance_list))
    ...
    def flops(self):
        ...
        flops += self.UpsampleWithGuidance.flops()
        return flops
